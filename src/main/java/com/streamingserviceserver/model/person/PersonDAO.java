...
public class PersonDAO implements Dao<Person> {
    ...
    public boolean add(Person person) {
        ...
        String sql = String.format(template, person.getPersonId(), person.getFirstName(), person.getMiddleName(), person.getLastName());
    ...
    public boolean update(Person person) {
        ...
        String template = "UPDATE " + DatabaseHandler.TABLE_PERSON + " SET " +
                DatabaseHandler.COLUMN_PERSON_FNAME + " = '%s' SET " +
                DatabaseHandler.COLUMN_PERSON_MNAME + " = '%s' SET " +
                DatabaseHandler.COLUMN_PERSON_LNAME + " = '%s' WHERE " +
                DatabaseHandler.COLUMN_PERSON_ID + " = '%s'";

        String sql = String.format(template, person.getFirstName(), person.getMiddleName(), person.getLastName(), person.getPersonId());
        return _sqlHandler.executeSql(sql);
    }
    ...
    private Person getFromResultSet(ResultSet rs) throws SQLException {
        ...
        return new Person(id, first, middle, last);
    }
